...
if __name__ == '__main__':
    ...
    flc = FacialLandmarkController(imageSize)
    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")

    ### Define Networks ###

    netG = pix2pixG.UnetGenerator(input_nc=config.INPUT_CHANNEL, output_nc=4,
                                  num_downs=int(math.log2(config.IMAGE_SIZE)), ngf=64,
        ...
        try:
            ...
            if config.TEST_INPUT == "Camera":
                frame = car.cropAndResizeImageDatasetBased(frame, imageSize, np.array(
                    [ frame.shape[1]/2-frame.shape[0]/2, frame.shape[1]/2+frame.shape[0]/2, 0, frame.shape[0]-1])) #[480, 480 + 248, 260, 260 +282]
